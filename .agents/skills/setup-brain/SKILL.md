...
5. Treat `CLAUDE.md` as the vault's shared identity layer. When Codex needs the user's long-term identity, goals, current projects, or collaboration preferences, read `## Who I am`, `## This year's goals`, `## Collaboration preferences`, and `## Current projects` there. `AGENTS.md` is a symlink to `CLAUDE.md`; never edit it directly.
