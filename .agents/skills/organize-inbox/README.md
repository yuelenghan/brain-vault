...
当前 brain 的整理逻辑有多套 AI 工具入口：`.claude/skills/organize-inbox/SKILL.md` 服务 Claude Code/headless 且作为 canonical 规则源，`.agents/skills/organize-inbox/SKILL.md`、`.codex/skills/organize-inbox/SKILL.md` 与 `.copilot/skills/organize-inbox/SKILL.md` 都是薄入口并读取 `.claude` 规则。Codex 会话内仍由 `.agents` 这份 skill 触发，但运行时必须转读 `.claude` canonical。CronCreate 定时和 `.claude/organize.py` 离线兜底也由 `.claude` 副本承接。修 skill、脚本或离线整理入口后，必须用临时 vault 跑 `.claude/organize.sh`（macOS / Linux）或 `.claude/organize.ps1`（Windows）验证，覆盖"实际移动 + 提交 + 日志"完整路径，不要只看 Inbox 是否为空。

`.agents/skills/organize-inbox/` 只维护 Codex 会话触发与转发说明，不拥有整理脚本、eval 或独立流程；`.claude/` 是 Claude Code/headless 运行副本，维护 `organize.py`、`organize.sh`、`organize.ps1`、`skills/`、`bin/`；`.codex/`、`.copilot/`、`.github/copilot-instructions.md` 也在当前 vault 维护，但只作为 CLI 入口，不拥有整理逻辑。
