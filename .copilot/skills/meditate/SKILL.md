...
4. 如果用户明确要求 `nightly` 或 `weekly` cadence，且你是当前 Copilot agent，不要运行 `.claude/meditate.sh`；它是外层 Claude Code/headless cadence 入口，可能递归拉起另一个 agent。继续按 canonical `.claude` skill 的底层 deterministic script / wrapper 流程执行，再做验证与结果汇报。
