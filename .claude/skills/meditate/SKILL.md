...
- **Projects/ content misplacement**: a top-level `Projects/<dir>/` whose name begins with an ownership project stem but is not the ownership note's own same-name directory is merged under that project; a content note under `Projects/<dir>/` whose stem belongs to a different project is rehomed (with a matching subdirectory when one exists). A note can also rehome when a unique Project is the explicit subject of its plan/design title or its non-negated runtime/delivery evidence; integration, research, comparison, and explicitly excluded dependencies are weaker/non-ownership signals. Equally strong Project evidence is report-only—never resolve it by alphabetical order. The ownership note + same-name material directory pattern (e.g. `primary-project.md` + `primary-project/`) is the kept PARA pattern and is not restructured.
...
- The script's `apply-safe` downgrading handle-wikilinks ([[ @handle ]], causal target begins with @, no real target note) to plain text
- The script's `apply-safe` deleting 0-byte empty stubs only after fixing all causal wikilinks that caused them
- The script's `apply-safe` deleting 0-byte handle-wikilink stubs after downgrading their causal [[@handle]] links to plain text
...
- The script's `apply-safe` merging a stray top-level `Projects/<dir>/` under `Projects/<project>/<dir>/` via `git mv` (directory move), rehoming misplaced `Projects/<dir>/<note>.md` content notes to the owning project (with a matching subdirectory when one exists), repairing path-qualified wikilinks to the filename stem, and syncing plain-text `承接 Projects/...` path references in ownership notes
...
- Every Projects/ directory-merge and note-rehome target exists; plain-text `承接 Projects/...` path references in ownership notes now point at the new paths; bare wikilinks still resolve (directory moves do not change filename stems).
