...
- **frontmatter value validation**: the scan report's `invalid_frontmatter` lists notes whose `---` fences are on line 1 but an unquoted scalar value contains `: ` (colon+space) — YAML parses this as a nested mapping and Obsidian silently drops all properties / Dataview. `fix_frontmatter.py` does **not** fix this (it is structural-only); for non-protected notes, fix by hand: double-quote the offending value (e.g. `source: "DeepSeek-AI 论文（DSpark: Confidence ...）"`). Quoted strings, flow collections, list items, and `sha256:abc` (colon with no space) are not flagged.
