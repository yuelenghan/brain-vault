...
当前 brain 的整理逻辑有多套 AI 工具入口：`.claude/skills/ingest/SKILL.md` 服务 Claude Code/headless 且作为 canonical 规则源，`.agents/skills/ingest/SKILL.md`、`.codex/skills/ingest/SKILL.md` 与 `.copilot/skills/ingest/SKILL.md` 都是薄入口并读取 `.claude` 规则。Claude 会话内手动、CronCreate 定时、`.claude/ingest.py` 离线兜底都调 `.claude` 这份 skill，平台入口是 `.claude/ingest.sh`（macOS / Linux）与 `.claude/ingest.ps1`（Windows）；Codex 会话内由 `.agents` 入口触发后也必须转读 `.claude` canonical。修 skill、脚本或离线整理入口后，必须用临时 vault 跑 `.claude/ingest.sh` 或 `.claude/ingest.ps1` 验证，覆盖"实际移动 + 提交 + 日志"完整路径，不要只看 Inbox 是否为空。

`.claude/` 是当前 vault 的 Claude Code/headless 运行副本，版本化 `ingest.py`、`ingest.sh`、`ingest.ps1`、`skills/`、`bin/`，其余（`ingest.log`、`scheduled_tasks.json` 等）被 `.gitignore` 忽略，均为本地态；`.agents/skills/ingest/`、`.codex/`、`.copilot/`、`.github/copilot-instructions.md` 也在当前 vault 维护，但只作为入口，不拥有整理逻辑。
