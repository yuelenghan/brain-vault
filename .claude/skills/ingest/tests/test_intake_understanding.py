...
class IntakeUnderstandingTest(unittest.TestCase):
    ...
    def test_existing_project_match_can_be_first_pass_target_without_area_gate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_note(
                vault / "Inbox" / "Primary Project Sprint Notes.md",
                "Primary Project Sprint Notes",
                "reference",
                "primary-project sprint notes for Jira automation and remediation workflow delivery.",
            )

            report = ingest.make_report(vault, convert=False)
            markdown = ingest.markdown_report(report)

        hint = report["understanding_hints"]["Inbox/Primary Project Sprint Notes.md"]
        self.assertEqual("Projects/primary-project/Primary Project Sprint Notes.md", hint["target_candidates"][0]["target"])
        self.assertEqual("Projects", hint["target_candidates"][0]["scope"])
        self.assertEqual("Projects/primary-project.md", hint["ownership_candidates"][0]["path"])
        self.assertEqual([], hint["ownership_actions"])
        readiness = report["placement_readiness"]["Inbox/Primary Project Sprint Notes.md"]
        self.assertEqual("ready", readiness["status"])
        self.assertEqual(["Projects/primary-project.md"], readiness["ownership"])
        org_plan = report["organization_plan"]["Inbox/Primary Project Sprint Notes.md"]
        self.assertEqual("ready", org_plan["status"])
        self.assertFalse(org_plan["resource_index"]["required"])
        seed = report["distillation_seed"]["Inbox/Primary Project Sprint Notes.md"]
        self.assertEqual("Projects/primary-project", seed["use_context"]["target_dir"])
        content_plan = report["content_patch_plan"]["Inbox/Primary Project Sprint Notes.md"]
        self.assertIn("`Projects/primary-project`", content_plan["body_markdown"])
        self.assertNotIn("待定主题", content_plan["body_markdown"])
        self.assertIn("Projects/primary-project/Primary Project Sprint Notes.md", markdown)
