...
- **`author` must be plain text, never a `[[wikilink]]`**: upstream clippers may wrap social handles as `author: ["[[@Vercantez]]"]`; a wikilink to a non-existent `@handle.md` becomes an unresolved graph node (grey dot) and clicking it creates a 0-byte stub. Strip the `[[...]]` wrapping to `@Vercantez` plain text. The preprocessor's `clean_author_value` and `verify_wikilinks.py`'s frontmatter-author check enforce this.
...
- If the report marks a candidate `salience: high`, treat it as stronger encoding: prefer 5-7 key points rather than 3-5, persist `salience: high` in frontmatter, and include one explicit bullet or sentence on the material's direct impact on current projects.
- The manual salience marker `#重要` is exact, not fuzzy: it triggers only when the first non-empty content line is exactly `#重要`. Variants such as `# 重要笔记`, later headings, or inline mentions do not count.
